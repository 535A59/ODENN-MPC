...
class ODEMPC:
    ...
    @torch.no_grad()
    def simulate_plant_one_step(self, x_current_nn, u_applied):
...
if __name__ == '__main__':
    ...
    mpc_controller = ODEMPC(
        ...
        scaler_x_path='scaler_x.joblib', scaler_u_path='scaler_u.joblib',
        state_dim=odenn_state_dim, input_dim=input_dim, N=N_horizon, Ts=Ts,
        Q=Q_cost, R=R_cost, optimizer="adam", lr=0.001, optim_iter=5, device=DEVICE
    )

    x_nn_current = np.zeros(odenn_state_dim) 

    history_x_nn = [x_nn_current]
    history_u = []
    history_ref_output = [ref_output_trajectory[0, :]]

    print("--- 开始 MPC 仿真循环 ---")
    start_time = time.time()
    for k in tqdm(range(total_steps)):
        X_ref_future_np = get_future_xref(
            ref_output_trajectory, k, N_horizon, odenn_state_dim, output_indices_in_nn_state
        )
        U_optimal_sequence = mpc_controller.solve_ocp(x_nn_current, X_ref_future_np)
        u_apply = U_optimal_sequence[0, :]

        x_nn_next = mpc_controller.simulate_plant_one_step(x_nn_current, u_apply)

        x_nn_current = x_nn_next
        
        history_x_nn.append(x_nn_current)
        history_u.append(u_apply)
        history_ref_output.append(ref_output_trajectory[k+1, :])
        mpc_controller._warm_start()

    end_time = time.time()
    print(f"--- sim finished ---")
    print(f"total time: {end_time - start_time:.2f} 秒")
    print(f"average time: {(end_time - start_time) / total_steps * 1000:.2f} 毫秒")

    history_x_physics = np.array(history_x_nn)
    history_u = np.array(history_u)
    history_ref_output = np.array(history_ref_output)
    time_axis = np.arange(total_steps + 1) * Ts

    history_q3_deg = history_x_physics[:, 0] * 180 / np.pi
    history_q4_deg = history_x_physics[:, 1] * 180 / np.pi
    ref_q3_deg = history_ref_output[:, 0] * 180 / np.pi
    ref_q4_deg = history_ref_output[:, 1] * 180 / np.pi

    plt.figure(figsize=(12, 10))
    plt.suptitle("MPC Tracking Performance (Tuned for Target)", fontsize=16)

    # q3 Plot
    plt.subplot(3, 1, 1)
    plt.plot(time_axis, ref_q3_deg, 'k-', label='q3_ref')
    plt.plot(time_axis, history_q3_deg, 'r--', label='q3_sim')
    plt.title("q3 Tracking")
    plt.ylabel("q3 [°]")
    plt.legend()
    plt.grid(True)
    ...
    plt.plot(time_axis, history_q4_deg, 'r--', label='q4_sim')
    plt.title("q4 Tracking")
    plt.ylabel("q4 [°]")
    plt.legend()
    plt.grid(True)
    ...
    # Control Input Plot
    plt.subplot(3, 1, 3)
    time_axis_u = np.arange(total_steps) * Ts
    plt.plot(time_axis_u, history_u[:, 0], label='u1 [Nm]')
    plt.plot(time_axis_u, history_u[:, 1], label='u2 [Nm]')
